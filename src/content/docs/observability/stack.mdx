...
A fully custom dashboard tracks the backup and storage posture of the cluster:

| Panel | Type | What It Shows |
|---|---|---|
| Backup Status | Stat | OK/FAILED per backup job (color-coded) |
| Time Since Last Backup | Stat | Green < 25h, yellow < 48h, red > 48h |
| Backup Duration | Bar chart | Historical duration per job |
| Backup Files (New + Changed) | Stacked bars | File change volume per run |
| PostgreSQL Dump Size | Line chart | pg_dumpall file size over time |
| OCI Bucket Usage vs Limit | Gauge | Used vs 20 GB OCI free tier |
| OCI Bucket Size by Prefix | Pie chart (donut) | Breakdown by backup prefix |
| OCI Bucket Objects by Prefix | Bar gauge | Object count per prefix |

{/* IMAGE: Custom Grafana dashboard showing backup status panels and OCI bucket usage gauge */}
