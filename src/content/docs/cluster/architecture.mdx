...
{/* IMAGE: Full-screen photo of the mini PC running the cluster (musashi node) */}
...
The CNI plugin is **Cilium**, running as a DaemonSet in `kube-system`. Cilium was chosen for several reasons:

- **eBPF-based dataplane** — replaces kube-proxy entirely for routing and load balancing, reducing overhead and improving observability
- **Native network policies** — Cilium enforces NetworkPolicy CRDs and extends them with CiliumNetworkPolicy
- **Built-in Hubble** — deep flow visibility without additional tooling
- **Ingress controller integration** — Cilium provides a `cilium-ingress` LoadBalancer service on ports 80/443
...
{/* IMAGE: Screenshot of Traefik dashboard showing active routes and services */}
...
{/* IMAGE: Screenshot of cert-manager certificates in ArgoCD or kubectl output */}
...
```
              ...
              (systemd, host)
                       │
    ┌──────────────────┴────────────────────┐
    │           musashi (192.168.1.133)     │
    │  Kubernetes v1.34.3 / containerd      │
    │                                       │
    │  ┌─────────────────────────────────┐  │
    │  │   Traefik (Gateway API)         │  │
    │  │   :30080 / :30443               │  │
...
| `kube-system` | cilium (DaemonSet), cilium-operator | eBPF networking and network policy |
...
- Photo of the mini PC hardware running the homelab (musashi node)
- Screenshot of Traefik dashboard showing active routes and services
- Screenshot of cert-manager certificates list (kubectl output or ArgoCD)
- Screenshot of `kubectl get nodes -o wide` showing musashi node status
