...
and the screenshots are real, and cannot be re-captured.

Worth knowing what it cost while it ran: eight pods — core, registry, nginx, portal,
jobservice, PostgreSQL, Redis and Trivy — on a single 16 GB node, mirroring images that CI
was already pushing to GHCR. Replication and lifecycle policies earn that footprint at scale;
with one machine and one pipeline, they were capacity spent on capability that was not being
used.
